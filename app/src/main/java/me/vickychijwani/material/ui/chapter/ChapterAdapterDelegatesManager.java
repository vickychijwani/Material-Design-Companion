...
// NOTE: code adapted from Hannes Dorfmann's AdapterDelegates library
 ...
 * #addDelegate(ChapterAdapterDelegate)}
 * </p>
 *
 * <p>
 ...
 * #setFallbackDelegate(ChapterAdapterDelegate)} that will be used if no {@link AdapterDelegate} is
...
class ChapterAdapterDelegatesManager {

    /**
     * Map for ViewType to AdapterDeleage
     */
    protected SparseArrayCompat<ChapterAdapterDelegate> delegates = new SparseArrayCompat<>();
    private ChapterAdapterDelegate fallbackDelegate;

    /**
     * Adds an {@link AdapterDelegate}. Internally calls {@link #addDelegate(ChapterAdapterDelegate,
     ...
     * @see #addDelegate(ChapterAdapterDelegate, boolean)
     */
    public ChapterAdapterDelegatesManager addDelegate(@NonNull ChapterAdapterDelegate delegate) {
     ...
     * @see #setFallbackDelegate(ChapterAdapterDelegate)
     */
    public ChapterAdapterDelegatesManager addDelegate(@NonNull ChapterAdapterDelegate delegate,
     ...
     * @return self
     */
    public ChapterAdapterDelegatesManager removeDelegate(@NonNull ChapterAdapterDelegate delegate) {
        ChapterAdapterDelegate queried = delegates.get(delegate.getItemViewType());
     ...
     * @return self
     */
    public ChapterAdapterDelegatesManager removeDelegate(int viewType) {
     ...
     */
    public int getItemViewType(@NonNull List<Object> items, int position) {
        int delegatesCount = delegates.size();
        for (int i = 0; i < delegatesCount; i++) {
            ChapterAdapterDelegate delegate = delegates.valueAt(i);
    ...
    @NonNull public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ChapterAdapterDelegate delegate = delegates.get(viewType);
        if (delegate == null) {
            if (fallbackDelegate == null) {
    ...
    public void onBindViewHolder(@NonNull List<Object> items, int position,
        ...
        ChapterAdapterDelegate delegate = delegates.get(viewHolder.getItemViewType());
        if (delegate == null) {
            if (fallbackDelegate == null) {
     ...
     */
    public ChapterAdapterDelegatesManager setFallbackDelegate(
            @Nullable ChapterAdapterDelegate fallbackDelegate) {
